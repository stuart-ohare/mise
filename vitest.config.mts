...
export default defineConfig({
  resolve: {
    alias: {
      ...
      // Next substitutes `server-only` at compile time and never resolves the npm
      // package, so Vitest has to do the same to load a module that carries the marker.
      // This is the empty module Next's own client compiler swaps in. It costs nothing:
      // the marker's job is to fail a client bundle, and Vitest is Node.
